#!/usr/bin/env python
"""
Migrate question and solution data directly from the file system to Firestore.
This script reads markdown files and uploads them to Firestore collections.
"""
import os
import re
import glob
...
async def process_solution_file(file_path: str, db: FirestoreClient) -> Dict:
    ...
    try:
        ...
        solution_data = {
            ...
            'solveFramework': sections.get('solve_framework', ''),  # Update from bucesr_framework if needed
...
async def process_categories(db: FirestoreClient, question_paths: List[str]) -> None:
    ...
    try:
        ...
        for category_path in sorted(category_paths):
            ...
            category_id = category_path.replace(os.path.sep, '_')
