...
class Question:
    ...
    category: str
    subcategory: str
    similar_questions: List[str]  # Changed back to List[str]
    ...
    for line in content.split('\n'):
        if line.strip().startswith('-'):
            ...
            if len(key_value) == 2:
                ...
                key = key.strip().strip('**')
                value = value.strip()
                if key in ['Similar Questions', 'Real Life Domains']:
...
def get_question(question_id: int) -> Optional[Question]:
    ...
    try:
        ...
        question = Question(
            ...
            real_life_domains=metadata.get('Real Life Domains', []),
...
def get_questions_list(category: str = None, difficulty: str = None, company: str = None,  limit=None) -> Tuple[List[Question], int]:
