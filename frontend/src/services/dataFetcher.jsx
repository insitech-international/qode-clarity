class DataFetcher {
  ...
  async getCategories() {
    ...
    // Production mode: ensure index.json is loaded
    if (this.contentPaths.questions.size === 0) {
      await this.loadContentPaths();
    }

    // Extract categories from question paths
    const categories = Array.from(this.contentPaths.questions.values())
      .map(path => {
        const parts = path.split('/');
        return parts[3]; // "Advanced_Python_Concepts" from "static/data/questions/Advanced_Python_Concepts/..."
      })
      .filter(Boolean);
